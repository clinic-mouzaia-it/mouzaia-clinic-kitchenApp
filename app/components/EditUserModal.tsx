...
const EditUserModal: React.FC<EditUserModalProps> = ({
  ...
  const [departement, setDepartement] = useState(user.departement); // NEW state
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      onUserUpdated({ ...user, fullName, departement, position, level }); // include departement here
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md space-y-4 border border-gray-200">
        ...
        <div className="space-y-3">
          ...
          />
          <input
            value={departement} // NEW input for departement
